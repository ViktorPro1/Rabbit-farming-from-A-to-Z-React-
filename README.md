# 🐰 Кролівництво від А до Я

Веб-додаток про кролівництво — від базових знань до практичних порад. Проєкт створений для тих, хто хоче почати або розвивати власне господарство.

## 🔗 Live Demo

👉 https://rabbit-farming-from-a-to-z-react.vercel.app/

---

## 📌 Опис проєкту

Це інформаційний SPA-додаток, створений на базі React, який надає структуровану інформацію про:

- утримання кролів
- розведення
- годування
- вибір порід
- базові помилки та поради

Проєкт орієнтований на простий та зрозумілий UX.

---

## ⚙️ Технології

- React
- JavaScript (ES6+)
- CSS / SCSS
- Vite / CRA
- Vercel

---

## 🚀 Основний функціонал

- Структурований контент по темах
- Адаптивний дизайн
- Швидке завантаження
- Зручна навігація
- Компонентна архітектура

---

## 📂 Структура проєкту

src/
components/ — UI компоненти  
pages/ — сторінки  
assets/ — зображення  
data/ — контент  
App.jsx  
main.jsx

---

## 🛠️ Встановлення та запуск

Клонувати:
git clone https://github.com/your-username/rabbit-farming.git

Перейти:
cd rabbit-farming

Встановити:
npm install

Запуск:
npm run dev

---

## 📦 Збірка

npm run build

---

## 📚 Документація для розробників

- `docs/DEVELOPER_GUIDE.md` — швидкий гід для запуску, CI, релізів і правил компонентів.

---

## 🌍 Деплой

Vercel:

- авто деплой
- CDN
- HTTPS

---

## 🎯 Мета

- Зручний довідник по кролівництву
- Демонстрація React навичок
- Основа для масштабування

---

## 🔮 Покращення

- Мультимовність

---

## 📄 Ліцензія

Проєкт ліцензовано за умовами MIT. Дивись також:

- `LICENSE` (англійською)
- `LICENSE.uk.md` (українською)

---

## 👨‍💻 Автор

Viktor Pro1

Замітка: цей проект належить Viktor Pro1.

Розроблено вручну у VS Code з підтримкою AI-інструментів.
